...
export const deleteDocument = async (id: number): Promise<{ message: string; id: number }> => {
    const response = await axios.delete<{ message: string; id: number }>(`/api/documents/${id}`);
    return response.data;
};
